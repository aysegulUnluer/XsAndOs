...
struct GameView: View {
    ...
    @EnvironmentObject var connectoinManager: MPConnectionManager
    ...
    var body: some View {
        VStack{
            ...
            HStack {
                Button(game.player1.name){
                    ...
                    if game.gameType == .peer{
                        ...
                        connectoinManager.send(gameMove: gameMove)
                    }
                }
            ...
            .disabled(game.boardDisabled ||
                      game.gameType == .peer &&
                      connectoinManager.myPeerId.displayName != game.currentPlayer.name)
            VStack {
                if game.gameOver {
                    Text("Game Over")
        ...
        .onAppear {
            game.reset()
            if game.gameType == .peer{
                connectoinManager.setup(game: game)
            }
        }
        .inNavigationStack()
    }
